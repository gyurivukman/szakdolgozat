...
class AESFileCipher():
    ...
    def encryptFile(self, file_name):
        ...
        normal_part_size = math.ceil(file_size / self.__part_count) # 11/3 -> 4
        ...
        with open(file_name, 'rb') as source:
            for key, index in zip(self.__keys, range(0, self.__part_count)):
                ...
                normal_chunk_count = math.ceil(normal_part_size / AES.block_size)
                with open(file_part_name, 'wb') as target:
                    target.write(cipher.iv)
                    if index < self.__part_count - 1:
                        for _ in range(0, normal_chunk_count):
                            raw_chunk = source.read(AES.block_size)
...
if __name__ == "__main__":
    ...
    keys = [b"AESEncryptionKey", b"LowFastKavitssal", b"AESEncryptionKey", b"SEAEncryptionKey", b"SEAEncryptionYek", b"SEAEncrypoitnKey", b"SEAEncryptionKey", b"AESEncryptionKey", b"AESEncryptionKey", b"AESEncryptionKey", b"AESEncryptionKey"]
